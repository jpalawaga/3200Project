...
// Problem number: 5734 - Shape Number
// Description: I find the difference array and then use suffix arrays to sort
// the double of the difference array and the first suffix in the suffix array
// that has the first index less than the size of the original array is the
// answer
//*****************************************************************************
/*
 * Suffix array
...
char difference(const char & a, const char& b) {
   if(a < b) {
      return (char)((b - '0') - (a - '0') + '0');
   } else if( a == b) {
      return '0';
   } else {
      return (char)(8 - (a - '0') + (b - '0') +'0');
   }
}

string differenceArray(const string &a) {
   string diff = a;
   for(int i = 0; i < a.length(); ++i) {
      diff[i] = difference(a[i], a[(i + 1) % a.length()]);
   }
   return diff;
}
...
int main(void)
{
	...
	cout << str.substr(sarray[i], loop) << endl;
     }
